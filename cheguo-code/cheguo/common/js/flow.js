...
function flowSubmit(_url1,_url2,_url3,_data,financeChannel){
    var _financeChannel = financeChannel ? {financeChannel:financeChannel} : '';
    comn.ajax({
        url: _url1,
        data: $.extend(_data,_financeChannel),
        success: function (res0) {
            ...
            if (_url1 === interUrl.myTask.pledgeoverdueLaunch) { // 如果是超抵合同流程多传一个参数
                p3.businessObjectId = res0.data.businessObjectId
            }
            if(res0.data.userTasks.length>1){
                ...
                tableEvent_sign = {
                    "click .role": function (e, a, item, index) {
                        p2 = {nextNodeUserName: item.userName, nextNodeUserId: a}
                        if (_url1 === interUrl.myTask.pledgeoverdueLaunch) { // 如果是超抵合同流程多传一个参数
                            p2.businessObjectId = res0.data.businessObjectId
                        }
                    }
                };
...
//车信贷相关方法start
//车信贷-流程退回上一步
function flowBack3Pre(){
    //退回上一步
    comn.ajax({
        url: interUrl.carCreditLoan.back2pre,
        data: loanApplyId,
        success: function (res1) {
            tip({content: res1.message});
            comn.closeTab();
...
//车信贷-关闭贷款
function flowClose2LoanApply(){
    $("#sureModal").modal("hide");
    comn.ajax({
        url: interUrl.carCreditLoan.closeLoanApply,
        data: loanApplyId,
        success: function (res) {
            tip({content: res.message});
            comn.closeTab();
...
//微车贷相关方法start
//微车贷-流程退回上一步
function flowBack4Pre(){
    //退回上一步
    comn.ajax({
        url: interUrl.miniCarLoan.back2pre,
        data: loanApplyId,
        success: function (res1) {
            tip({content: res1.message});
            comn.closeTab();
...
//微车贷-关闭贷款
function flowClose3LoanApply(){
    $("#sureModal").modal("hide");
    comn.ajax({
        url: interUrl.miniCarLoan.closeLoanApply,
        data: loanApplyId,
        success: function (res) {
            tip({content: res.message});
            comn.closeTab();
