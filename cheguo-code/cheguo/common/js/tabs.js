...
menuItemClick = function() {
  ...
  if (add) {
    //针对后缀不需要新增时间戳的数据
    var isDel = false;
    if (param && param.indexOf('deleteT=yes') > 0) {
        param = param.replace('&deleteT=yes', '');
        isDel = true;
    }
    menuTab = "<a href = 'javascript:;' class = 'active J_menuTab' data-id= '" + url + "'> " + title + " <i class = 'fa'>×</i> </a>";
    $(".J_menuTab").removeClass("active");
    iframe = "<iframe class = 'J_iframe' name='iframe_" + index + "' width='100%' height='100%' src='" + url + "?" + (param ? param + "&" : "") + (isDel ? '' : ("t=" + (new Date().getTime()))) + "' frameborder='0' data-id='" + url + "' seamless></iframe>";
...
tabRefresh = function(o) {
  ...
  if(o.title) {
    $(".J_menuTab[data-id='" + (o.url || $(this).data("id")) + "']").html(o.title + " <i class = 'fa'>×</i> </a>")
  }
...
$(function() {
  toastr.options = {
    ...
    }).on("mouseleave", ".J_menuTab", function(){
