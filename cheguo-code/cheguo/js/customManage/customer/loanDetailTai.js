var args, base64, dataLoad_1, dataLoad_2, getDocumentList, handle_1, handle_2, targetDir, treeObj, zTreeOnClick,dataLoad_4;
...
$("#print").click(function(){
  ...
  //借款人信息
comn.ajax({
  url: interUrl.tempLoanQuery.getLoanCustomerInfo,
...
$("#table_guarantor").bootstrapTable(comn.table);
  //汽车信息
  comn.ajax({
    url: interUrl.tempLoanQuery.getLoanCarInfoAndLicensePlateInfo,
    data: {
      projectId: args['projectId']
    },
    success: function(res) {
      ...
      var day1 = new Date();day1.setDate(day1.getDate() - 1);
      var day2 = new Date(res.data.registerDate) > new Date() ? day1.format("yyyy-MM-dd") : res.data.registerDate;
      $('input[name="registerDate"]').val(day2)
...
dataLoad_opinion = function (params) {
	tableData(
		...
		interUrl.tempLoanQuery.flowLoan
	);
};
...
$(function () {
  ...
  //总揽信息
  comn.ajax({
    url: interUrl.tempLoanQuery.getLoanInfoOverview,
    data: {
      projectId: args['projectId']
    },
    success: function(res) {
      ...
      res.data.getLoanInfoOverview && $("input[name='repaymentNo']").parents('.form-group-sm').removeClass('hide');
    }
  });
