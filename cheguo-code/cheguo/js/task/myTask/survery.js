...
$(function () {
	...
	comn.ajax({
		...
		success: function (res) {
		  ...
		  $("#getPos").data("pos", res.data.visitAddressLongitude + "," + res.data.visitAddressLatitude);
            ...
            if (res.data.econtractStatus === 2 || res.data.econtractStatus === 5) {
                $("input[name=isEcontract]").prop("disabled", true);
            } else {
                $("input[name=isEcontract]").prop("disabled", false);
            }
		}
	});
	...
	$("#btn-opinion-save").click(function () {
		var _econtractStatus = $("#econtractStatus").val();
		var _isEcontract = $("input[name=isEcontract]:checked").val();
		if (((_econtractStatus === "0" || _econtractStatus === "1" || _econtractStatus === "3" || _econtractStatus === "4") && _isEcontract === "1") || ((_econtractStatus === "2" || _econtractStatus === "5") && _isEcontract === "0")) {
			return tip({content: "该客户贷款还未完成合同签订，请使用车贷APP完成签约或返回取消线上签约！"});
		}
		...
		if($("#opinionForm").valid() == true){
			...
			$("#sureOption").unbind("click").click(function () {
				comn.ajax({
				  	url: interUrl.common.saveEContract,
				  	data: {
                        econtractStatus: $("#econtractStatus").val(),
                        isEcontract: $("input[name=isEcontract]:checked").val(),
                        loanApplyId: args["loanApplyId"]
				  	},
				  	success: function (res) {
					  //保存流程意见
