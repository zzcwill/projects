...
table_1 = function(params) {
    var p = params.data;
    p['status'] = 1;
    tableData(params, $.extend($("#loanInsuranceForm").values(), p), interUrl.insurance.loanInsuranceList);
};
...
tableEvent_1 = {
    ...
    "click .info": function(e, a, item, index) {
        return comn.addTab({
            href: "./Modal/customManage/customer/loanDetail.html?id="+item.projectId+"&customerId="+item.customerId+"&loanApplyId="+item.relativeApplyId1+"&businessTypeCode=" + item.flowType+"&projectId="+item.projectId+"&space=LOAN&releventFlowNode=LOAN_QUERY&releventFlow=LOAN_QUERY",
            title: '查看贷款项目'
        })
    }
