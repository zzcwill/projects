...
tableEvent_1 = {
	...
	"click .approve": function(e, a, item, index) {
	  comn.addTab({
		  title: '提交费用申请',
	    	href: "./Modal/finaceManage/costLaunch/costInfo.html?type=approve&feeId=" + item.id + "&feeType=tip"
	  });
	},
...
handle_1 = function(value, row, index) {
	if (row.status == 0) {
		return ['<a class="lauchFee" href="javascript:;">发起费用申请</a>'].join("");
	}
...
$(function(){
	...
	$("#register").click(function(){    
	    comn.addTab({
			title: '发起费用申请',
			href: './Modal/finaceManage/costLaunch/costInfo.html?t=' + (new Date().getTime()) + "&feeType=tip"
		});
	});
});
