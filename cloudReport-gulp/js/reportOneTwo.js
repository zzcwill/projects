$(function () {
  ...
  //信用评分和欺诈与风险选中框边框自适应
  function creditInfoChooseBorderAuto2() {
    var length = $('.scorePartTwo .scoreTipTwo').length;
    var length2 = $('.scorePartTwo .scoreTip2Two').length;
    var length3 = $('.scorePartTwo .scoreTip3Two').length;
    for (var i = 0; i < length; i++) {
      var width = $('.scorePartTwo .scoreTipTwo').eq(i).parent('.scorePartTwo').width();
      var scoreTipWidth = (width - 4) + 'px';
      $('.scorePartTwo .scoreTipTwo').eq(i).css('width', scoreTipWidth);
    }
    for (var j = 0; j < length2; j++) {
      var width = $('.scorePartTwo .scoreTip2Two').eq(j).parent('.scorePartTwo').width();
      var scoreTipWidth = (width - 4) + 'px';
      $('.scorePartTwo .scoreTip2Two').eq(j).css('width', scoreTipWidth);
    }
    for (var k = 0; k < length3; k++) {
      var width = $('.scorePartTwo .scoreTip3Two').eq(k).parent('.scorePartTwo').width();
      var scoreTipWidth = (width - 4) + 'px';
      $('.scorePartTwo .scoreTip3Two').eq(k).css('width', scoreTipWidth);
    }
  }   
  //查无此结果显示
  ...
  //跟页面逻辑相关start
  //综合结论左边图标展示和结论展示
  function showEndResults(conclusion, result, decisionDate) {
    //一次综合决策
    ...
    if(decisionDate !== '') {
      $('#time').html('生成时间：' + decisionDate);
    }    
  }
  function showEndResults2(conclusion, result, yunTuScore, decisionDate) {
    //一次综合决策
    function onlyOneResults(dataOnlyOneResults) {
      if (dataOnlyOneResults === 1) {
        $('#resultsTipTwo').text(yunTuScore);
        $('#resultsTwo').addClass('resultsColor');
        $('#resultsImgTwo').attr('src', './img/index/1.png');
        return;
      }

      if (dataOnlyOneResults === 0) {
        $('#resultsTwo').addClass('resultsColor3');
        $('#resultsImgTwo').attr('src', './img/index/3.png');
        $('#resultsTipTwo').text(yunTuScore);
        return;
      }
    }
    ...
    $('#automatedEndTwo').html('自动化审批结果：' + result);

    if(decisionDate !== '') {
      $('#timeTwo').html('生成时间：' + decisionDate);
    }
  }
  ...
  function getFinalReport2() {
    function changeTitleAndId2(index) {
      ...
      $('#reportListTwo .reportTwo').eq(index).children('.titleTwo').eq(0).children('span').eq(0).text(title + name + idNo);
    } 

    //改变demo报告信用评分和欺诈评分内容
    function changeCreditInfo2(index) {
      //当前个人报告对象
      ...
      if (data.brCreditScoreDetail && data.brCreditScoreDetail.show) {
        ...
        var dom = $('.scoreListTwo').eq(scoreListAdd).children('.scorePartTwo');
        var scoreTxt = data.brCreditScoreDetail.creditScore;
        if (data.brCreditScoreDetail.level === '1') {
          dom.eq(0).children('.scoreTipTwo').removeClass('dn');
          dom.eq(0).children('.scoreTipTwo').eq(0).children('span').text(scoreTxt);
          //dom.eq(0).children('.icoTriangleTwo').removeClass('dn');
          dom.eq(0).addClass('choose');
        } else if (data.brCreditScoreDetail.level === '2') {
          dom.eq(1).children('.scoreTip2Two').removeClass('dn');
          dom.eq(1).children('.scoreTip2Two').eq(0).children('span').text(scoreTxt);
          //dom.eq(1).children('.icoTriangleTwo').removeClass('dn');
          dom.eq(1).addClass('choose2');
        } else if (data.brCreditScoreDetail.level === '3') {
          dom.eq(2).children('.scoreTip3Two').removeClass('dn');
          dom.eq(2).children('.scoreTip3Two').eq(0).children('span').text(scoreTxt);
          //dom.eq(2).children('.icoTriangleTwo').removeClass('dn');
      ...
      if (data.pinganCreditScoreDetail && data.pinganCreditScoreDetail.show) {
        ...
        var dom2 = $('.scoreListTwo').eq(scoreListAdd2).children('.scorePartTwo');
        var scoreTxt2 = data.pinganCreditScoreDetail.creditScore;
        if (data.pinganCreditScoreDetail.level === '1') {
          dom2.eq(0).children('.scoreTipTwo').removeClass('dn');
          dom2.eq(0).children('.scoreTipTwo').eq(0).children('span').text(scoreTxt2);
          //dom2.eq(0).children('.icoTriangleTwo').removeClass('dn');
          dom2.eq(0).addClass('choose');
        } else if (data.pinganCreditScoreDetail.level === '2') {
          dom2.eq(1).children('.scoreTip2Two').removeClass('dn');
          dom2.eq(1).children('.scoreTip2Two').eq(0).children('span').text(scoreTxt2);
          //dom2.eq(1).children('.icoTriangleTwo').removeClass('dn');
          dom2.eq(1).addClass('choose2');
        } else if (data.pinganCreditScoreDetail.level === '3') {
          dom2.eq(2).children('.scoreTip3Two').removeClass('dn');
          dom2.eq(2).children('.scoreTip3Two').eq(0).children('span').text(scoreTxt2);
          //dom2.eq(2).children('.icoTriangleTwo').removeClass('dn');
      ...
      if (data.brCheatScoreDetail && data.brCheatScoreDetail.show) {
        ...
        var dom3 = $('.scoreListTwo').eq(scoreListAdd3).children('.scorePartTwo');
        var scoreTxt3 = data.brCheatScoreDetail.cheatScore;
        if (data.brCheatScoreDetail.level === '1') {
          dom3.eq(0).children('.scoreTip3Two').removeClass('dn');
          dom3.eq(0).children('.scoreTip3Two').eq(0).children('span').text(scoreTxt3);
          //dom3.eq(0).children('.icoTriangleTwo').removeClass('dn');
          dom3.eq(0).addClass('choose3');
        } else if (data.brCheatScoreDetail.level === '2') {
          dom3.eq(1).children('.scoreTipTwo').removeClass('dn');
          dom3.eq(1).children('.scoreTipTwo').eq(0).children('span').text(scoreTxt3);
          //dom3.eq(1).children('.icoTriangleTwo').removeClass('dn');
          dom3.eq(1).addClass('choose');
        } else {
          isHaveArr[2] = false;
        }
      } else {
      ...
      function anutoChangeScore(isHaveArr,index) {
        ...
        for(var j = 2; j < isHaveArr.length ; j++) {
          if(isHaveArr[j] === true) {
            rightNum = rightNum + 1;
          }
        }
        ...
        if(leftNum === 0 && rightNum === 0) {
          $('.creditBoxTwo').eq(0+index*2).addClass('dn');
          $('.creditBoxTwo').eq(1+index*2).addClass('dn');
        }

        if(leftNum === 0 && rightNum !== 0) {
          $('.creditBoxTwo').eq(0+index*2).addClass('dn');
          $('.creditBoxTwo').eq(1+index*2).removeClass('creditBox2Two');
        }   
        ...
        //每个人信用分模块左右两块高度start
        // if(leftNum === 1) {
        //   $('.creditScoreTwo').eq(0+index*2).addClass('creditScore2Two');
        // }              
        // if(rightNum === 1) {
        //   $('.creditScoreTwo').eq(1+index*2).addClass('creditScore2Two');
        // }                      
        //每个人信用分模块左右两块的高度end   
        ...
        for(var k = 0 ; k < isHaveArr.length ; k++ ) {
          if(isHaveArr[k] === false) {
            $('.score').eq(k+index*2).addClass('dn');
          }
        }
  ...
  function getHtml() {
    //第一份报告相关
    //综合结论相关
    showEndResults(conclusionData.typeConclusion1, conclusionData.result, conclusionData.decisionDate);
    ...
    //第二份报告相关start
    showEndResults2(conclusionData2.typeConclusion1, conclusionData2.result, conclusionData2.yunTuScore, conclusionData2.decisionDate);
    getFinalReport2();

    creditInfoChooseBorderAuto2();
    //第二份报告相关end
      ...
      success: function (res) {
        ...
        yjAjax({
          ...
          success: function (res2) {
            ...
            //for (var i = 0; i < res2.members.length; i++) {
            for (var i = 0; i < 1; i++) {
              getOnePersonReport2(res2.members[i].type);
            }
          }
        });
      }
