$(function () {
  ...
  //跟页面逻辑相关start
  //综合结论左边图标展示和结论展示
  function showEndResults(conclusion, result, yunTuScore, decisionDate) {
    //一次综合决策
    function onlyOneResults(dataOnlyOneResults) {
      if (dataOnlyOneResults === 1) {
        $('#resultsTip').text(yunTuScore);
    ...
    $('#automatedEnd').html('自动化审批结果：' + result);
    ...
    function changeCreditInfo(index) {
      ...
      if (data.brCheatScoreDetail && data.brCheatScoreDetail.show) {
        ...
        if (data.brCheatScoreDetail.level === '1') {
          dom3.eq(0).children('.scoreTip3').removeClass('dn');
          dom3.eq(0).children('.scoreTip3').eq(0).children('span').text(scoreTxt3);
          //dom3.eq(0).children('.icoTriangle').removeClass('dn');
          dom3.eq(0).addClass('choose3');
        } else if (data.brCheatScoreDetail.level === '2') {
          dom3.eq(1).children('.scoreTip').removeClass('dn');
          dom3.eq(1).children('.scoreTip').eq(0).children('span').text(scoreTxt3);
  ...
  function toShowReportHtml() {
    ...
    if (requestOkNumber === reportArrInfo.length) {
      ...
      //只显示主贷人
      resHtml = resHtml + resHtmlArr[0].html;
      // for (var i = 0; i < resHtmlArr.length; i++) {
  ...
  function getHtml() {
    //综合结论相关
    showEndResults(conclusionData.typeConclusion1, conclusionData.result, conclusionData.yunTuScore, conclusionData.decisionDate);
    //showRiskResults(personsRisks);

    //生成右边固定导航条
    //只显示主贷人
    var reportArrInfo2 = [reportArrInfo[0]];
    getMenuList(reportArrInfo2);
  ...
  function getCloudMirrorReport() {
    ...
    yjAjax({
      ...
      success: function (res) {
        ...
        conclusionData.yunTuScore = res.detail.yunTuScore || 0;
