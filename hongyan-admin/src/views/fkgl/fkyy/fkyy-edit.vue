<template>
  <div class="fkyy-edit-page">
      ...
      <div class="flex-center w-fit-content">
        ...
        >
          <div class="form-line">
            ...
            <el-form-item label="所属部门：" prop="visitorDepartment">
              <el-input v-model="form.visitorDepartment" placeholder="请输入"></el-input>
            </el-form-item>

            <el-form-item label="预约来访日期：" prop="visitorTime">
              <el-date-picker
            ...
            <el-form-item label="来访事由分类：" prop="visitorTypeName">
              <el-select v-model="form.visitorTypeName" placeholder="请选择" style="width: 200px" @change="changeItem">
                <el-option
    ...
    <div v-if="addCarDialog" style="margin-top: 100px">
      ...
      >
        <div>
          <fkyy-car-detail :view="viewData" :type="type" @save="addCar"></fkyy-car-detail>
        </div>
      </el-dialog>
    </div>
    <div v-if="showDialog" style="margin-top: 100px">
      ...
      >
        <div>
          <fkyy-visitor-detail :view="viewData" :type="type" @save="addVisitor"></fkyy-visitor-detail>
        </div>
      </el-dialog>
    </div>
  </div>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        ...
        rules: { // 必填字段校验
          ...
          visitorTypeName: [
            { required: true, message: '请选择来访事由分类', trigger: 'blur' }
          ]
        }
      }
    },
    ...
    methods: {
      typeNameTrans(typeCode) {
        for (let i = 0; i < this.options.length; i++) {
          if (this.options[i].typeCode === typeCode) {
            this.form.visitorTypeName = this.options[i].typeName
            break
          }
        }
      },
      changeItem(item) {
        this.form.visitorTypeCode = item
        this.typeNameTrans(item)
        console.log('code:' + this.form.visitorTypeCode + ',name:' + this.form.visitorTypeName)
      },
      changeEmployee(item) {
