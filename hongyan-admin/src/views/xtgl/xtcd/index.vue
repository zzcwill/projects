<template>
  <div class="container">
    <el-card class="box-card">
      <el-button style="float:left" type="primary" @click="creatMenuListDialog">新建</el-button>
      ...
      <el-upload class="upload-demo" :headers="myHeaders" :action="baseUrl +'/api/1.0/xtgl/menu/uploadExcel'"
      ...
      <el-button style="margin-left:10px" type="primary" @click="searchList()" icon="el-icon-search">检索</el-button>
    </el-card>
    ...
    <el-card class="box-card box-card-bd">
      ...
      <el-table :data="tableData" border style="width: 100%">
        ...
        <el-table-column prop="fatherMenuId" label="上级菜单id">
        </el-table-column>
        <el-table-column prop="menuName" label="菜单名称">
        </el-table-column>
        <el-table-column prop="menuLevel" label="菜单级别">
        </el-table-column>
        <el-table-column prop="menuType" label="菜单类别">
        </el-table-column>
        <el-table-column prop="menuUrl" label="菜案内容">
        </el-table-column>
        <el-table-column prop="menuSortNum" label="排列顺序">
        </el-table-column>
        <el-table-column prop="address" label="操作" width='220'>
          <template slot-scope="scope">
            <el-button type="text"
              ...
              编辑</el-button>
            <el-button type="text" @click="open(scope.row.id,1)">删除</el-button>
            <el-button type="text" @click="open(scope.row.id,2)">停用</el-button>
          </template>
        </el-table-column>
      </el-table>
      ...
      <!-- 新建菜单的弹框 -->
      <el-dialog title="新建" :visible.sync="dialogFormVisibleCreat">
        <el-form :model="formCreate" :rules="rulesCreat" ref="ruleFormCreat">
          <el-form-item label="上级菜单" :label-width="formLabelWidth" prop="fatherMenuId">
            <el-select v-model="formCreate.fatherMenuId" clearable placeholder="请选择">
              <el-option v-for="item in menuListOptions" :key="item.value" :label="item.label" :value="item.value">
              </el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="菜单级别" :label-width="formLabelWidth" prop="menuLevel">
...
<script>
  ...
  export default {
    ...
    destoryed() {},
    methods: {
      async creatMenuListDialog(){
          this.dialogFormVisibleCreat=true
          const res=await topMenuList()
          // console.log(res);
          res.data.forEach(element => {
            let obj={}
            obj.label=element.menuName
            obj.value=element.id
            this.menuListOptions.push(obj)
          });
      },
      ...
      open(id, flag) {
        if (flag == 1) {
          ...
          this.$confirm('此操作将永久停用数据, 是否继续?', '停用', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'
          }).then(async () => {
