  ...
  >
    <div>
      <el-form ref="ruleForm" :model="ruleForm" label-width="100px" class="demo-ruleForm">
        <el-form-item label="报修人：">{{ ruleForm.proposeName }}</el-form-item>
        <el-form-item label="报修时间：">{{ ruleForm.createTime }}</el-form-item>
        <el-form-item label="状态：" prop="patrolCycle">
          <el-select v-model="ruleForm.statusCode" placeholder="请选择状态">
            <el-option v-for="(item, index) in $store.state.reflex.wybxStaus" :key="index" :label="item.label" :value="item.value" />
          </el-select>
        </el-form-item>
        <el-form-item label="维修内容：">
          {{ ruleForm.proposeContent }}
          <!-- <el-table
            :border="true"
            :data="tableData"
            tooltip-effect="dark"
        ...
        <el-form-item label="处理情况：">
          <el-input v-model="ruleForm.replyContent" type="textarea" :autosize="{ minRows: 4, maxRows: 5 }"></el-input>
        </el-form-item>
      </el-form>
    </div>
