  ...
  <div style="height: 450px">
    <div style="width: 50%;float: left">
      <el-form ref="form" size="mini" :model="form" label-width="130px" :rules="rules" :disabled="isView" inline>
        ...
        <el-form-item label="会议类型：" prop="meetingTypeName">
          <el-select v-model="form.meetingTypeName" placeholder="请选择" @change="changeTypeItem" style="width: 360px">
            <el-option
    ...
    <div style="text-align: left;">
      <div style="font-size: 20px">已签到人数: {{signNum}}人</div>
      <div style="font-size: 20px;margin-top: 50px">签到二维码</div>
      <div class="app-container">
        <qr-code :text="signUrl"></qr-code>
      </div>
      <div style="text-align: right;">
        <el-button size="medium" type="primary" @click="save">关闭</el-button>
      </div>
    </div>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    created() {
      this.getConferenceModelOptions()
      if (this.apply) {
    ...
    methods: {
      ...
      changeTypeItem(val) {
        if (val === '早会') {
          this.form.meetingTypeCode = '1'
        } else if (val === '午会') {
          this.form.meetingTypeCode = '2'
        } else if (val === '晚会') {
          this.form.meetingTypeCode = '3'
        } else if (val === '周例会') {
          this.form.meetingTypeCode = '4'
        } else if (val === '临时例会') {
          this.form.meetingTypeCode = '5'
        } else if (val === '其他') {
          this.form.meetingTypeCode = '6'
        }
      },
      // 会议室模板选择
      changeItem(item) {
      ...
      getConferenceModelOptions() {
        queryConferenceAllModel().then(res => {
