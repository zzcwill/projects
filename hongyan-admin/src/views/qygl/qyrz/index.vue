    ...
    <div class="p-10 bg-white brs-4 def-shadow">
      ...
      <div>
        ...
        >
          ...
          <el-table-column prop="areaName" label="入驻区域">
            <div slot-scope="scope">
              {{`${scope.row.parkName}/${scope.row.buildingName}/${scope.row.floorName}/${scope.row.areaName}`}}
            </div>
          </el-table-column>
          <el-table-column prop="startTime" label="入驻开始时间"></el-table-column>
          <el-table-column prop="endTime" label="入驻结束时间"></el-table-column>
...
<script>
  import { listCmpentry, delCmpentry } from '@/api/qygl/qyrz'
  ...
  export default {
    ...
    methods: {
      ...
      query() {
        listCmpentry(this.form).then(res => {
          this.list = res.data
